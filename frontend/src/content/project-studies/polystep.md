...
- **정책 처리·검증 파이프라인:** 수집된 정책을 정규화하고 검증 상태를 관리하는 백엔드 흐름을 설계·구현했습니다.
- **공고 원문 확인:** Playwright와 browser-use로 기관 공고에 직접 접속해 정책명과 내용을 대조하고, 접속 실패·공고 없음·확인 필요를 구분해 기록했습니다.
- **팀 내 역할:** 팀장으로 개발 순서를 조율했습니다. 프론트엔드 화면과 원천 데이터 수집기는 팀원이 담당했습니다.
