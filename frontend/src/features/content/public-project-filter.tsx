...
export function ProjectFilter({ items }: { items: Item[] }) {
  ...
  return <>
    ...
    {visible.length ? <div className="grid gap-5">{visible.map(({ slug, card }) => <div key={slug}>{card}</div>)}</div> : <p className="border-y border-line py-7 text-sm text-muted">선택한 기술의 프로젝트가 없습니다.</p>}
  </>;
}
