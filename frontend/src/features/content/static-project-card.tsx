...
export function StaticProjectCard({ project }: { project: PortfolioProject }) {
  return <article className="grid gap-5 py-7 md:grid-cols-[minmax(0,1.35fr)_minmax(15rem,0.65fr)] md:items-start md:gap-10">
    <div>
      <h3 className="text-xl font-semibold leading-snug tracking-[-0.025em] sm:text-2xl"><Link className="rounded-sm decoration-accent decoration-2 underline-offset-4 hover:underline" href={`/projects/${project.slug}`}>{project.title}</Link></h3>
